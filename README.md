...
| `pi` | `p`, `pi-agent`, `pi-coding-agent` | `pi -p "<prompt>"` |
...
**Pi** — `pi -p`. `--provider` and plain or provider-qualified `--model` are passed through. `--output-format text` → `--mode text`; `json` / `stream-json` → `--mode json` (Pi's JSONL event stream). Pi has no permission-bypass flag, so `--yolo` is a no-op. Sessions: `--resume-id latest` → `--continue`; a specific `--resume-id` → `--session <id>`; `--session-id` stays `--session-id`. Pi sessions are discovered under its configured agent directory (by default `~/.pi/agent/sessions/`) and can be used as cross-agent transcript context. No MCP config flag.
